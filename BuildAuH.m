function [A] = BuildAuH(N,XU,YU,XV,YV,u_coeffs,Re,dt)
% INPUTS:
...
% --- Boundary Conditions ------------------------------------------------
% Unpack coeffs. for BCs: u_coeffs = [a1 ... a4; b1 ... b4];
a1 = u_coeffs(1,1); a2 = u_coeffs(1,2);
a3 = u_coeffs(1,3); a4 = u_coeffs(1,4); 
b1 = u_coeffs(2,1); b2 = u_coeffs(2,2);
b3 = u_coeffs(2,3); b4 = u_coeffs(2,4);
...
for j_ctr = 1:Nsml
    ...
    row(ctr) = j_ctr;
    col(ctr) = row(ctr);
    A(ctr) = a2 - b2/(YU(2,j)-YU(1,j));
    
    % North
    ctr = ctr + 1;
    row(ctr) = j_ctr;
    col(ctr) = row(ctr) + Nsml;
    A(ctr) = b2/(YU(2,j)-YU(1,j));
end
