function [A] = BuildAvH(N,XU,YU,XV,YV,v_coeffs,Re,dt)
% INPUTS:
...
c1 = v_coeffs(1,1); c2 = v_coeffs(1,2); 
c3 = v_coeffs(1,3); c4 = v_coeffs(1,4);
d1 = v_coeffs(2,1); d2 = v_coeffs(2,2);
d3 = v_coeffs(2,3); d4 = v_coeffs(2,4);
...
for j = [1 : NxL+2, NxM+1 : Nx+2]
    if j < NxL + 3 % bottom left bc
        ...
        row(ctr) = j;
        col(ctr) = row(ctr);
        A(ctr) = c2 - d2/( YV(2,j) - YV(1,j) );
        
        % North
        ctr = ctr + 1;
        row(ctr) = j;
        col(ctr) = row(ctr) + Nsml;
        A(ctr) = d2/( YV(2,j) - YV(1,j) );
        ...
        row(ctr) = j_ctr;
        col(ctr) = row(ctr);
        A(ctr) = c4 - d4/( YV(2,j) - YV(1,j) );
        
        % North
        ctr = ctr + 1;
        row(ctr) = j_ctr;
        col(ctr) = row(ctr)+Nsml;
        A(ctr) = d4/( YV(2,j) - YV(1,j) );
    end
end

% % Bottom Boundaries (\partial \Omega_i, i = 2, 4)
% dy = ( YV(3,1) - YV(1,1) ) * ( YV(2,1)-YV(1,1) ) * ( YV(2,1) - YV(3,1) );
% 
% for j_ctr = 1:Nsml
%     
%     % Principal - Boundary
...
%     row(ctr) = j_ctr;
%     col(ctr) = row(ctr);
%     A(ctr) = c2 - d2*( (YV(2,1)-YV(1,1))^2 - (YV(3,1)-YV(1,1) )^2) /dy;
%     
%     % North
%     ctr = ctr + 1;
%     row(ctr) = j_ctr;
%     col(ctr) = row(ctr) + Nsml;
%     A(ctr) = -d2*(YV(3,1)-YV(1,1) )^2/dy;
%     
%     % North North
%     ctr = ctr + 1;
%     row(ctr) = j_ctr;
%     col(ctr) = row(ctr) + 2*Nsml;
%     A(ctr) = d2*(YV(2,1)-YV(1,1) )^2/dy;
% end
